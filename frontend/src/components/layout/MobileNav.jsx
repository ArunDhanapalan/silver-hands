  ...
  return (
    ...
    >
      <div className="flex items-center justify-around">
        ...
        <Link 
          to="/" 
          className={`flex flex-col items-center justify-center min-w-[56px] min-h-[48px] py-1 text-[11px] font-medium rounded-lg ${
        ...
        {user?.role === 'senior' ? (
          <Link 
            to="/senior" 
            className={`flex flex-col items-center justify-center min-w-[56px] min-h-[48px] py-1 text-[11px] font-medium rounded-lg ${
              location.pathname.startsWith('/senior') ? 'text-primary font-bold' : 'text-base-content/70'
            }`}
            aria-label="Opportunities"
          >
            <Layers className="w-5 h-5 mb-0.5 text-warning" />
            <span>Deck</span>
          </Link>
        ) : user?.role === 'company' ? (
          <Link 
